...
public class Reporter : MonoBehaviour
{
	...
	public void Initialize()
	{
		...
		collapse = false;// (PlayerPrefs.GetInt("Reporter_collapse") == 1) ? true : false;
		...
		showClearOnNewSceneLoadedButton = false; //(PlayerPrefs.GetInt("Reporter_showClearOnNewSceneLoadedButton", 1) == 1) ? true : false;
		showTimeButton = false;// (PlayerPrefs.GetInt("Reporter_showTimeButton", 1) == 1) ? true : false;
		showSceneButton = false; // (PlayerPrefs.GetInt("Reporter_showSceneButton", 1) == 1) ? true : false;
        ...
        showCopyAllButton = false;// (PlayerPrefs.GetInt("Reporter_showCopyAllButton", 1) == 1) ? true : false;
	...
	void DrawInfo()
	{
		...
		GUILayout.Space(size.x);
		GUILayout.Label("Screen Height " + Screen.safeArea.height, nonStyle, GUILayout.Height(size.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
	...
	void DrawReport()
	{
		...
		screenRect.height = Screen.safeArea.height;
		GUILayout.BeginArea(screenRect, backStyle);
	...
	void drawToolBar()
	{
		...
		toolBarRect.x = 0f;
		toolBarRect.y = Screen.safeArea.yMin;
		...
		GUI.skin = toolbarScrollerSkin;
		Vector2 drag = getDrag();
		if ((drag.x != 0) && (downPos != Vector2.zero) && (downPos.y > Screen.height - size.y * 2f - Screen.safeArea.yMin)) {
	...
	void DrawLogs()
	{
		...
		int totalVisibleCount = (int)(Screen.safeArea.height * 0.75f / size.y);
		...
		buttomRect.x = 0f;
		buttomRect.y = Screen.safeArea.height - size.y;
	...
	void drawGraph()
	{
		...
		graphRect = stackRect;
		graphRect.height = Screen.safeArea.height * 0.25f;//- size.y ;
	...
	public void OnGUIDraw()
	{
		...
		screenRect.x = 0;
		screenRect.y = Screen.safeArea.yMin;
		screenRect.width = Screen.width;
		screenRect.height = Screen.safeArea.height;
		...
		logsRect.x = 0f;
		logsRect.y = Screen.safeArea.yMin + size.y * 2f;
		logsRect.width = Screen.width;
		logsRect.height = Screen.safeArea.height * 0.75f - size.y * 2f - Screen.safeArea.yMin;

		stackRectTopLeft.x = 0f;
		stackRect.x = 0f;
		stackRectTopLeft.y = Screen.safeArea.height * 0.75f;
		stackRect.y = Screen.safeArea.height * 0.75f;
		stackRect.width = Screen.width;
		stackRect.height = Screen.safeArea.height * 0.25f - size.y;



		detailRect.x = 0f;
		detailRect.y = Screen.safeArea.height - size.y * 3;
	...
	bool isGestureDone()
	{
		...
		int gestureBase = (Screen.width + (int)Screen.safeArea.height) / 4;
