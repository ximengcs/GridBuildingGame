...
public static class ShaderVariantCollector
{
    ...
    private const float WaitMilliseconds = 100f;
    ...
    {
        // 获取所有打包的资源
        CollectResult collectResult = null;// AssetBundleCollectorSettingData.Setting.GetPackageAssets(false, false,  _packageName);
        ...
        foreach (var collectAssetInfo in collectResult.CollectAssets)
        {
            ...
            if (collectAssetInfo.AssetInfo.AssetType == typeof(TMP_FontAsset))
            {
                string assetPath = collectAssetInfo.AssetInfo.AssetPath;
                if (result.Contains(assetPath) == false)
                    result.Add(assetPath);
